# `kubernetes_configmap` to `file` Example

Reads key-value pairs from a Kubernetes ConfigMap and writes them to a local JSON or YAML file.
...
| `destination.path` | Output file path (e.g. `"./output/k8s-configmap.json"`) |
